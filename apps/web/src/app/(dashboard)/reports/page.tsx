...
export default function ReportsPage() {
  ...
  const { isExporting, error: pdfError, exportPdf, clearError } = usePdfExport();
  ...
  return (
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (!el) return;
                const result = await exportPdf(el);
                if (result.success) {
                  addNotification({
                    id: `pdf-${Date.now()}`,
                    userId: currentUser?.id ?? "",
                    title: "PDF Exported",
                    message: `Report saved as report-${new Date().toISOString().slice(0, 10)}.pdf`,
                    isRead: false,
                    category: "report",
                    priority: "normal",
                  ...
                  addNotification({
                    ...
                    message: result.error ?? "Could not generate PDF. Please try again.",
                    isRead: false,
                    category: "system",
                    priority: "high",
                    createdAt: new Date().toISOString(),
            ...
            >
              <Printer className={cn("h-4 w-4 mr-1", isExporting && "animate-spin")} />
      ...
      {/* Report content - captured by html2canvas for PDF export */}
      <div ref={reportContentRef} className="space-y-6">
        {/* PDF report header - visible in exported PDF */}
        <div className="hidden print:block space-y-1 pb-4 border-b mb-4">
          <h1 className="text-2xl font-bold">Fleet Health Report</h1>
          <p className="text-sm text-muted-foreground">
            Date Range: {DATE_OPTIONS.find((d) => d.key === filter.dateRange)?.label ?? filter.dateRange}
            {filter.estateId && estateOptions.find((e) => e.id === filter.estateId)
              ? " - Estate: " + (estateOptions.find((e) => e.id === filter.estateId)?.name ?? "")
              : ""}
            {filter.siteId && siteOptions.find((s) => s.id === filter.siteId)
              ? " - Site: " + (siteOptions.find((s) => s.id === filter.siteId)?.name ?? "")
              : ""}
            {filter.deviceId && deviceOptions.find((d) => d.id === filter.deviceId)
              ? " - Device: " + (deviceOptions.find((d) => d.id === filter.deviceId)?.name ?? "")
              : ""}
          </p>
